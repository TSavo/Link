...
(function() {
  ...
  LinkPublisher = (function() {
    ...
    LinkPublisher.prototype.getMessageCost = function(addresses) {
      return ((addresses.length * 0.02) + (addresses.length * 0.00000001)).toFixed(8);
    };
    ...
    LinkPublisher.prototype.publish = function(message, callback) {
      ...
      return client.listUnspent(0, function(err, unspent) {
        ...
        for (_j = 0, _len1 = unspent.length; _j < _len1; _j++) {
          tx = unspent[_j];
          if (tx.amount.toFixed(8) >= total.toFixed(8) && ((useable == null) || useable.amount.toFixed(8) > tx.amount.toFixed(8))) {
            useable = tx;
          }
        }
        if (useable == null) {
        ...
        if (useable.amount.toFixed(8) > total.toFixed(8)) {
          outs[useable.address] = useable.amount.toFixed(8) - total.toFixed(8);
        }
